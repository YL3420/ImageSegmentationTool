...
public class GraphCut {
    ...
    public static void main(String[] args){
        ...
        GraphCut graph = new GraphCut(n, 9); // 9 edges total

        // Set terminal weights
        graph.setTerminalWeights(s, Float.POSITIVE_INFINITY, 0); // source
        graph.setTerminalWeights(t, 0, Float.POSITIVE_INFINITY); // sink

        // Source edges
        graph.setEdgeWeight(s, 0, 10);
        graph.setEdgeWeight(s, 1, 10);

        // Sink edges
        graph.setEdgeWeight(2, t, 10);
        graph.setEdgeWeight(3, t, 10);

        // Middle edges
        graph.setEdgeWeight(0, 1, 2);
        graph.setEdgeWeight(0, 2, 4);
        graph.setEdgeWeight(0, 3, 8);
        graph.setEdgeWeight(1, 3, 9);
        graph.setEdgeWeight(3, 2, 6);

        // Run the algorithm
        float maxFlow = graph.computeMaximumFlow(false, null);
        System.out.println("Max flow: " + maxFlow); // Expect: 19
    }
}